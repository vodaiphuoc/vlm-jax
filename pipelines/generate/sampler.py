...
class _SamplingState:
    ...
    num_input_tokens: INT32 = flax.struct.field(pytree_node=False)
...
def _sample_top_p(
    ...
    r"""Sample a token using top-p sampling."""
...
class Sampler:
    ...
    def transformer_state(self, state: statelib.State) -> None:
        ...
        def check_tree_structure(tree1, tree2):
            ...
            def _check_shape_dtype_sharding(x, y):
                return (
            ...
            if not all(
                    jax.tree_util.tree_leaves(
                        jax.tree_util.tree_map(_check_shape_dtype_sharding, tree1, tree2)
                    )
                ):
                raise ValueError(
    ...
    def _sample_step(
        ...
        r"""Performs a single sampling step."""
